...
mcp-router installs Model Context Protocol servers from registries or npm, runs
them as lazily-spawned stdio child processes (or proxies remote
...
client — and give each client exactly the tools it needs. Manage everything
through the built-in React web UI or hand-edit the flat JSON config and reload,
with bearer-token auth in front.
